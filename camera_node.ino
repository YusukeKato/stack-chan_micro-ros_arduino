...
void camera_timer_callback(rcl_timer_t * timer, int64_t last_call_time) {
  if (timer != NULL) {
    ...
    if (fb) {
      ...
      bool jpeg_converted = fmt2jpg(fb->buf, fb->len, fb->width, fb->height, fb->format, 30, &out_jpg, &out_jpg_len);
      if (jpeg_converted && out_jpg != NULL) {
        ...
        /*
        rcl_ret_t ret = rcl_publish(&camera_pub, &image_msg, NULL);
        if (ret != RCL_RET_OK) {
          Serial.printf("[WARN] Publish failed. Error: %d, Size: %d bytes\n", ret, out_jpg_len);
        }
        */
        free(out_jpg);
      } else {
